#!/usr/bin/env bash
#
# mayhem/test.sh — RUN Hyperion's behavioral pytest oracle (deps already installed by
# mayhem/build.sh) and emit a CTRF (ctrf.io) summary. exit 0 iff failed==0.
# PATCH-grade oracle: a no-op patch that neuters the obfuscator breaks the known-answer
# assertions in test_hyperion_oracle.py, so it FAILS here (anti-reward-hacking).
#
# It does NOT compile — build.sh installed pytest + atheris + pystyle into the in-image
# site dir and compiled the hyperion_run_tests ELF wrapper. We only RUN the suite.
set -uo pipefail
...
# Put the in-image site dir (atheris + pytest + pystyle) + hyperion source on PYTHONPATH.
...
"$RUNNER" -p no:cacheprovider -o addopts= -q \
    mayhem/test_hyperion_oracle.py 2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}

# Parse pytest's summary line, e.g. "8 passed in 0.4s" / "1 failed, 7 passed in ...".
